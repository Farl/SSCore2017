...
{
    [UnitCategory("SS")]
    [UnitTitle("Request System Message")]
    [UnitShortTitle("Request System Message")]
    public class SystemMessageUnit : Unit, IGraphElementWithData
    {
        ...
        [DoNotSerialize, PortLabel("Type")] public ValueInput messageTypeInput;
        ...
        protected override void Definition()
        {
            ...
            messageTypeInput = ValueInput<string>(nameof(messageTypeInput), string.Empty);
        ...
        protected virtual ControlOutput OnInput(Flow flow)
        {
            ...
            var buttonSetups = new SystemMessageUI.ButtonSetup[texts.Count];
            for (int i = 0; i < texts.Count; i++)
            {
                var index = i;
                buttonSetups[i] = new SystemMessageUI.ButtonSetup
                {
                    buttonText = texts[i],
                    onButton = () =>
                    {
            ...
            var requestData = new SystemMessageUI.RequestData
            {
                typeName = flow.GetValue<string>(messageTypeInput),
            ...
            var handle = SystemMessageUI.Request(requestData);
            var data = flow.stack.GetElementData<Data>(this);
            data.buttonIndex = -1;
            data.handle = handle;
            return output;
        ...
        {
            var data = flow.stack.GetElementData<Data>(this);
            SystemMessageUI.Close(data.handle);
            return null;
        }
